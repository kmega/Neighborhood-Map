...
const SearchOption = props => {
  const { places, query, toggle, showSearchBox } = props;
  return (
    <nav id="nav" className={toggle}>
      ...
      <ul role="menu" className="place-list">
          ...
          .map(place => {
            return (
              <li
                ...
                className="place"
                key={place.name}
                onClick={props.showShop.bind(this, place.name)}
                onKeyPress={props.showShop.bind(this, place.name)}
              >
                {place.name}
              </li>
            );
